...
s1negloc = [(s1posloc(1)) (s1posloc(2)+10)];
...
tau = ceil(0.5/fmax*gx);             %Duration
t0 = abs(ceil(5*tau));                 %Delay / Offset for no pops or clicks

%Compute source 

source1(1, t0 : t0 + ceil(T/dt) - 1) = ((10^-12)*10^(srcA(1)/20)).*exp(-((T - t0)/tau).^2);  %Gaussian equationmakes impulse in array
% t0 = t0 + 10;
source2(1, t0 : t0 + ceil(T/dt) - 1) = ((10^-12)*10^(srcA(2)/20)).*-exp(-((T - t0)/tau).^2);  %Gaussian equationmakes impulse in array
